...
function CheckMatchingError(errorMsg) {
    const value = errorMsg
    console.log("Error msg Recieved: ", value)
    const regex = /reverted with custom error '(.+?)'/
    const match = value.match(regex)
    if (match) {
        const customError = match[1]
        console.log(customError) // Output: `Error`()
        return customError
    }
    return null
}
...
!developmentChains.includes(network.name)
    ...
    : describe("random Ipfs NFT unit test", () => {
          ...
          describe("Constructor", () => {
              it("Inilizes the Random IPFS NFT correctly.", async () => {
          ...
          describe("getBreedFromModdedRng", () => {
              ...
              it("should revert if moddedRng > 99", async function () {
                  ...
                  ).to.be.revertedWith("RandomIpfsNFT__RangeOutOfBounce")
              })
          })
      })
